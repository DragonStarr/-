...
def test_any_non_local_environment_requires_real_runtime_secrets() -> None:
    ...
    try:
        ...
        message = str(exc)
        assert "ALLOW_DEMO_AUTH" in message
        assert "TOKEN_ENCRYPTION_KEY" in message
    else:
        raise AssertionError("non-local settings must require real runtime secrets")
...
def test_token_cipher_requires_explicit_key_or_local_scope() -> None:
    try:
        TokenCipher("")
    except ValueError as exc:
        assert "TOKEN_ENCRYPTION_KEY" in str(exc)
    else:
        raise AssertionError("empty token cipher key must require an explicit local scope")


def test_token_cipher_uses_explicit_local_scope_without_exposing_plaintext() -> None:
    cipher = TokenCipher("", local_fallback_scope="test")
