...
- Readiness теперь считает `marketplace_api_verification` блокером по статусу account validation, а не по всем optional capability-флагам отдельных модулей; статус разделяет `ready_for_replay_pilot`, `blocked_for_live_pilot` и `ready_for_live_pilot`.
...
- Live-sync каталога сохраняет строки в `products`; `DatabaseReplayHub` в API/Telegram сначала берет tenant-данные из БД и только при пустой БД падает в replay.
