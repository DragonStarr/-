...
class AccountRepository:
    def __init__(self, session: AsyncSession) -> None:
        ...
        self._cipher = TokenCipher(
            ...
            local_fallback_scope=settings.app_env if settings.is_local_env() else None,
        )
    ...
    async def connect_account(
        ...
        *,
        ...
        stored_metadata = _secure_account_metadata(metadata, self._cipher)
        row = Account(
            tenant_id=ctx.tenant_id,
    ...
    async def missing_write_scope_verifications(self, ctx: TenantContext) -> list[str]:
        await bind_tenant_scope(self.session, ctx)
        rows = (
            await self.session.execute(
                select(Account).where(Account.tenant_id == ctx.tenant_id).order_by(Account.platform)
        ...
        missing: list[str] = []
        for row in rows:
            connected = self._to_connected(row)
            payload = row.payload or {}
            write_capabilities = payload.get("write_capabilities") or payload.get(
                "writeCapabilities"
            ) or {}
            if not isinstance(write_capabilities, dict):
                write_capabilities = {}
            for scope in ("catalog", "reviews", "ads"):
                if connected.capabilities.get(scope) != "ready":
                    continue
                if str(write_capabilities.get(scope) or "") != "ready":
                    missing.append(f"{row.platform}:{scope}")
        return missing

    async def mark_write_scopes_verified(
        self,
        ctx: TenantContext,
        *,
        account_id: str,
        ...
        if row.status != "validated":
            raise ValueError("read access must be validated before write scopes")
        allowed_scopes = {"catalog", "reviews", "ads"}
        normalized = sorted({scope for scope in scopes if scope in allowed_scopes})
        if not normalized:
            raise ValueError("write scopes are not supported for this account")
        ...
        for scope in normalized:
            write_capabilities[scope] = "ready"
        payload["write_capabilities"] = write_capabilities
        payload["write_scope_verification"] = {
            "scopes": normalized,
        ...
        self.session.add(
            AuditLog(
                tenant_id=ctx.tenant_id,
                user_id=ctx.user_id,
                action="account_write_scopes_verified",
                before_after={
    ...
    async def performance_credentials_for_account(
        self,
        ctx: TenantContext,
        account_id: str,
    ) -> MarketplaceCredentials:
        ...
        if row is None or row.tenant_id != ctx.tenant_id:
            raise KeyError(account_id)
        payload = row.payload or {}
        encrypted_secret = str(payload.get("performance_client_secret_enc") or "")
        if not encrypted_secret:
            return MarketplaceCredentials(
                platform="ozon_performance",
                api_key="",
                client_id=str(payload.get("performance_client_id") or ""),
                metadata=dict(payload),
            )
        return MarketplaceCredentials(
            platform="ozon_performance",
            api_key=self._cipher.decrypt(encrypted_secret),
            client_id=str(payload.get("performance_client_id") or ""),
            metadata=dict(payload),
        )
...
def _capabilities_for(platform: Platform, payload: dict) -> dict[str, str]:
    if platform == Platform.OZON:
        ...
        reviews_ready = bool(payload.get("review_management_subscription"))
        return {
            "catalog": "ready",
            "reviews": "ready" if reviews_ready else "needs_subscription",
            "finance": "ready",
    ...
    if platform == Platform.YANDEX:
        ...
        business_ready = bool(payload.get("business_id"))
        return {
            "catalog": "ready" if campaign_ready else "needs_credentials",
            "reviews": "needs_api_verification",
            "finance": "ready",
            "ads": "ready" if business_ready else "needs_credentials",
            "claims": "needs_api_verification",
        }
    return {"pvz": "ready"}


def _secure_account_metadata(metadata: dict, cipher: TokenCipher) -> dict:
    payload = dict(metadata)
    performance_secret = str(payload.pop("performance_client_secret", "") or "")
    if performance_secret:
        payload["performance_client_secret"] = True
        payload["performance_client_secret_enc"] = cipher.encrypt(performance_secret)
        payload["performance_client_secret_fingerprint"] = fingerprint_secret(performance_secret)
    else:
        payload["performance_client_secret"] = False
    return payload
...
class CatalogRepository:
    ...
    async def upsert_products(
        ...
        *,
        ...
        for item in rows:
            ...
            product_payload = _product_payload(source, item)
            commission_rate = _number(
                item.get("commission_rate")
                or item.get("commissionRate")
                or item.get("commission")
                or product_payload.get("commission_rate")
            )
            rating = _number(
                item.get("rating") or item.get("ratingValue") or product_payload.get("rating")
            )
            if existing is None:
                existing = Product(
                    tenant_id=ctx.tenant_id,
                ...
                existing.price = _number(item.get("price"), existing.price)
                existing.cost = _number(item.get("cost"), existing.cost)
                existing.commission_rate = commission_rate or existing.commission_rate or 0
                existing.rating = rating or existing.rating or 0
                existing.payload = {**(existing.payload or {}), **product_payload}
            await self._replace_stock_rows_if_present(ctx, existing, item)
            count += 1
        self.session.add(
            AuditLog(
                tenant_id=ctx.tenant_id,
    ...
    async def _replace_stock_rows_if_present(
        self,
        ctx: TenantContext,
        product: Product,
        item: dict,
    ) -> None:
        stock_rows = _stock_rows_from_product_item(item)
        if not stock_rows:
            return
        existing = (
            await self.session.execute(
                select(Stock).where(
                    Stock.tenant_id == ctx.tenant_id,
...
class SalesRepository:
    ...
    async def import_sales(
        ...
        *,
        ...
        for item in rows:
            sku = str(
                item.get("sku") or item.get("offer_id") or item.get("product_id") or ""
            ).strip()
            if not sku:
                continue
            platform = Platform(
                str(item.get("platform") or item.get("source") or Platform.OZON.value)
            )
            external_id = str(
                item.get("sale_id") or item.get("saleId") or item.get("id") or ""
            ).strip()
            sale_id = (
                _scoped_import_id(ctx, f"sale:{platform.value}", external_id)
                if external_id
                else str(uuid4())
            )
            row = await self.session.get(Sale, sale_id)
            if row is not None and row.tenant_id != ctx.tenant_id:
                raise KeyError(external_id or sale_id)
            if row is None:
                row = Sale(id=sale_id, tenant_id=ctx.tenant_id)
                self.session.add(row)
            row.account_id = account_id
            row.platform = platform.value
            row.sku = sku
            row.sold_at = _parse_datetime_value(
                item.get("sold_at") or item.get("soldAt") or item.get("date")
            )
            row.quantity = max(0, int(_number(item.get("quantity") or item.get("qty"), 1)))
            row.revenue = _number(item.get("revenue") or item.get("price") or item.get("amount"))
            row.cost = _number(item.get("cost"))
            row.payload = _product_payload(platform.value, item)
            count += 1
        self.session.add(
            AuditLog(
                tenant_id=ctx.tenant_id,
                ...
                before_after={"account_id": account_id, "count": count},
            )
        )
        await self.session.commit()
        return count
...
class OperationalRecordRepository:
    ...
    async def import_records(
        ...
        *,
        ...
        for item in rows:
            ...
            record_id = (
                _scoped_import_id(ctx, kind, external_id)
                if external_id
                else str(uuid4())
            )
            row = await self.session.get(model, record_id)
            if row is not None and row.tenant_id != ctx.tenant_id:
                raise KeyError(external_id or record_id)
            if row is None:
                row = model(id=record_id, tenant_id=ctx.tenant_id)
                self.session.add(row)
            row.product_id = str(
                item.get("sku")
            ...
            row.source = str(item.get("source") or kind)
            row.payload = _json_safe(
                {
                    **item,
                    "source": row.source,
                    "external_id": external_id or record_id,
        ...
        self.session.add(
            AuditLog(
                tenant_id=ctx.tenant_id,
                user_id=ctx.user_id,
                action="operational_records_imported",
                before_after={"kind": kind, "count": count},
            )
        )
        await self.session.commit()
        return count
    ...
    async def list_records(
        ...
        *,
        ...
        return [_operational_record_payload(row) for row in rows]


def _operational_model(kind: str):
    try:
        return OPERATIONAL_RECORD_MODELS[kind]
    except KeyError as exc:
        raise ValueError("unknown operational record kind") from exc


def _operational_record_payload(row: JsonTenantRecord) -> dict:
    payload = dict(row.payload or {})
    ...
    async def search(
        ...
        *,
        ...
        if _is_postgres_session(self.session):
            pgvector_rows = await self._search_pgvector(
                ctx,
        ...
        ranked = [
            (row, cosine_similarity(_vector_to_list(row.vector), query_vector))
            for row in rows
        ]
    ...
    async def _search_pgvector(
        ...
        *,
        ...
        result = await self.session.execute(
            text(
                """
                SELECT id, 1 - (vector <=> CAST(:query_vector AS vector)) AS score
        ...
        scored = [(str(row.id), float(row.score or 0)) for row in result.fetchall()]
        if not scored:
            return []
        rows = (
            await self.session.execute(
                select(SemanticMemory).where(SemanticMemory.id.in_([row[0] for row in scored]))
            )
        ).scalars().all()
        by_id = {row.id: row for row in rows}
        return [(by_id[row_id], score) for row_id, score in scored if row_id in by_id]
...
def _number(value, default: float = 0) -> float:
    ...
    if isinstance(value, dict):
        value = value.get("value") or value.get("current") or value.get("price") or default
    try:
        return float(value)
...
def _product_payload(source: str, item: dict) -> dict:
    safe = {
        key: value
...
def _stock_rows_from_product_item(item: dict) -> list[dict[str, int | str]]:
    ...
    if isinstance(raw_stocks, list):
        ...
        for stock in raw_stocks:
            ...
            warehouse = str(
                ...
                or stock.get("name")
                or stock.get("cluster")
                or "total"
            )
            rows.append(
                {
                    "warehouse": warehouse[:200],
                    "quantity": max(
                        0,
                        int(
                            _number(
        ...
        return rows
    for key in ("stock", "quantity", "available", "present"):
        if key in item:
            warehouse = str(item.get("warehouse") or item.get("warehouseName") or "total")[:200]
            return [
                {
                    "warehouse": warehouse,
                    "quantity": max(0, int(_number(item.get(key)))),
                }
            ]
    return []
...
def _is_postgres_session(session: AsyncSession) -> bool:
    ...
    return str(bind.url).startswith("postgresql") if bind is not None else False
